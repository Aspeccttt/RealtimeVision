...
public class FirstPersonController : MonoBehaviour
{
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity))
            {
                ...
                if (hit.collider.CompareTag("DataPoint"))
                {
                    ...
                    if (hit.collider.transform.parent.tag == "LineGraph")
                        ShowInfoLineGraphPanel(hit.point, dataPointColor, dataName, "Category", "Value", "Time");
    ...
    private void ShowInfoLineGraphPanel(Vector3 hitPoint, Color dataPointColor, string dataName, string CX, string CY, string CZ)
    {
        // Position the info panel relative to the camera
        Vector3 forwardDirection = playerCamera.transform.forward;
        forwardDirection.y = 0; // Adjust if you want the panel to align differently
